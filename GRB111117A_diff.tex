...
%DIF DEL GRB111117A_old2.tex   Thu Dec  7 10:01:32 2017
%DIF ADD GRB111117A.tex        Tue Feb  6 11:21:47 2018
...
%DIFDELCMD < It is notoriously difficult to localize short $\gamma$-ray bursts (sGRBs) and
%DIFDELCMD < their hosts to measure their redshifts. These measurements, however, are
%DIFDELCMD < critical to constrain the nature of sGRB progenitors, their redshift
%DIFDELCMD < distribution and the $r$-process element enrichment history of the universe.
%DIFDELCMD < Here, we present spectroscopy of the host galaxy of GRB~111117A and measure its
%DIFDELCMD < redshift to be $z = 2.211$. This makes GRB~111117A the most distant
%DIFDELCMD < high-confidence short duration GRB detected to date. Our spectroscopic redshift
%DIFDELCMD < supersedes a lower, previously estimated photometric redshift value for this
%DIFDELCMD < burst.
%DIFDELCMD < 

%DIFDELCMD < We use the spectroscopic redshift, as well as new imaging data to constrain the
%DIFDELCMD < nature of the host galaxy and the physical parameters of the GRB. The rest-frame
%DIFDELCMD < X-ray derived hydrogen column density, for example, is the highest compared to a
%DIFDELCMD < complete sample of sGRBs and seems to follow the evolution with redshift as
%DIFDELCMD < traced by the hosts of long GRBs.
%DIFDELCMD < 

%DIFDELCMD < The host lies in the brighter end of the expected sGRB host brightness
%DIFDELCMD < distribution at $z = 2.211$, and is actively forming stars. Using the observed
%DIFDELCMD < sGRB host luminosity distribution, we find that between 43 and 71 per cent of
%DIFDELCMD < all \textit{Swift}-detected sGRBs have too faint hosts at $z\sim2$ to allow for
%DIFDELCMD < a secure redshift determination. This implies that the measured sGRB redshift
%DIFDELCMD < distribution could be incomplete at high redshift. The large $z$ of
%DIFDELCMD < GRB~111117A is evidence against a lognormal delay-time model for sGRBs through
%DIFDELCMD < the predicted redshift distribution of sGRBs, which is very sensitive to
%DIFDELCMD < high-$z$ sGRBs.
%DIFDELCMD < 

%DIFDELCMD < From the age of the universe at the time of GRB explosion, an initial neutron
%DIFDELCMD < star (NS) separation of $a_0 < 3.1~R_\odot$ is required in the case where the
%DIFDELCMD < progenitor system is a circular pair of inspiralling NSs. This constraint
%DIFDELCMD < excludes some of the longest sGRB formation channels for this burst.
%DIFDELCMD < }
%DIFDELCMD < %%%
...
spectrum, we are able to put contraints on the escape fraction of \lya~in the sGRB
host.
